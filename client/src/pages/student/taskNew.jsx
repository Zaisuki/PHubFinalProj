...
>>>>>>> 532d8a9be91db44adccf061c289dfc5d80f09bdf
    };
    ...
    return (
        <Container>
            <Row>
                <div className='taskk-header'>
                    <h4 className='task-type'>
                        {classType.toUpperCase()}:‎ ‎<span className='task-titles'>{pageData.postTitle}</span>
                    </h4>{' '}
                </div>
<<<<<<< HEAD
...
>>>>>>> 532d8a9be91db44adccf061c289dfc5d80f09bdf
                            ...
                            {classType.toLowerCase() !== 'coach' && (
                                <h5 className='dueT'>
                                    {pageData.dueDate ? (
                                        <>
                                            Due on‎ ‎<span className='date'>{formatDate(pageData.dueDate)}</span>{' '}
                                        </>
                                    ) : (
                                        'No Due Date'
                                    )}
                                ...
                                <Row>
                                    {pageData.attachment.map((dataPage) => (
                                        <Col key={dataPage._id}>
                                            <div className='image-container'>{dataPage.type.startsWith('image') ? <ImagePreview className='attachment-image' imageUrl={dataPage.url} /> : <LinkPreview Url={dataPage.url} />}</div>
                                        </Col>
                                    ))}
                                </Row>
                            </Card>
                    ...
                    (!submitted ? (
                        <>
                            ...
                            {showWindow && (
                                <div className='popup-submission'>
                                    <FaTimes className='eks' onClick={handleCloseClick} />
                                    <FileUploader className='w-screen' handleChange={(files) => setAttachment((prevState) => [...prevState, ...Object.values(files)])} name='file' types={fileTypes} multiple={true} />
                                </div>
                            )}
                        </>
                    ) : (
                        ...
                        {submitted ? (
                            <div className='choice-container'>
                                <form>
                                    {pageData.postChoices.map((dataPage) => (
                                        <div key={dataPage._id}>
                                            {pageData.studentSubmission[0] && <input type='radio' disabled value={dataPage._id} checked={pageData.studentSubmission[0].answer._id === dataPage._id} />}
                                            <span>{dataPage.choice}</span>
